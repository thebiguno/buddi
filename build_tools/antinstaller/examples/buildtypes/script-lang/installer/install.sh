...
echo Set Local to Euskera  "y" or "n"
read EUSKERA
if [ "$EUSKERA" == "y" ] ; then
  JVM_PARAMS="-Duser.language=es -Duser.region=EU"
else
  echo Set Local to Spanish  "y" or "n"
  read SPANISH
  if [ "$SPANISH" == "y" ] ; then
    JVM_PARAMS="-Duser.language=es -Duser.region=ES"
  fi
fi

#echo $COMMAND -classpath $CLASSPATH  org.tp23.antinstaller.runtime.ExecInstall $1

$COMMAND $JVM_PARAMS -classpath $CLASSPATH  org.tp23.antinstaller.runtime.ExecInstall $GUI .
